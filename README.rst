...
Explore cube_array
^^^^^^^^^^^^^^^^^^
Explorecube _array is the new feature of PyOphidia. which the output of this operator is set of 4 arrays. " list of dimentions", " List of variables", " The dimentions parameters" and  " The variables parameters".

The Explorecube_array has intraction with numpy library  and Mathplotlib, to handle the mathematical operations and plot the final result.

Following is the example for Mathplotlib:

.. code-block:: python
    
    print(variables parameters[:24])
    print(variables parameters[25:48])
    plt.plot(variables parameters[:24], label='-4.5, 5.0')
    plt.plot(variables parameters[25:48], color='r', label='-4.5, 7.0')
    plt.legend()
    pl.title('Temperature for 24 hours for 2 locations', color='g')
    plt.xlabel('Time (h)')
    plt.ylabel('Temperature over surface (K)', color='r')
    plt.show()
