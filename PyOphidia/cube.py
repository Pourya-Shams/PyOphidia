...
class Cube():
    ...
    def explore_array(self, level=1, output_path='default', output_name='default', show_id='no', show_index='no', show_time='no', subset_dims=None, subset_filter=None, exec_mode='sync', ncores=1,
                      limit_filter=0, schedule=0, base64='yes', display=False):
        """explore(level=1, output_path='default', output_name='default', show_id='no', show_index='no', show_time='no', subset_dims=None, subset_filter=None, exec_mode='sync', ncores=1, base64='yes', display=False) ->
           dict or None : wrapper of the operator OPH_EXPLORECUBE
        ...
        def define_type(num, place):
            if place == 'float':
                num_elements = str(num) + 'f'
            if place == 'double':
                num_elements = str(num) + 'd'
            if place == 'int':
                num_elements = str(num) + 'i'
            if place == 'long':
                num_elements = str(num) + 'l'
            return num_elements

        def calculate_lengh(lengh, response_i):
            if response_i == 'float' or 'int':
                num = int(len(lengh)/4)
            if response_i == 'double' or 'long':
                num = int(len(lengh)/8)
            else:
                raise RuntimeError('The value type is not valid')
            return num
        # we count the number of dimentions in the JSON file.
        ...
        dim_data_1 = []
        m = []
        # Main function to loop inside JSON file an extract the parameters.
        if __name__ == '__main__':
            ...
            for response_i in response['response']:
                ...
                if response_i['objkey'] == 'explorecube_data' and response_i['objcontent'][0]['title'] == var_1:
                    i = response_i['objcontent'][0]['rowvalues'][0][2]
                    lengh = base64.b64decode(i)
                    if response_i['objcontent'][0]['rowfieldtypes'][2] == 'float' or 'int':
                        num = int(len(lengh)/4)
                    if response_i['objcontent'][0]['rowfieldtypes'][2] == 'double' or 'long':
                        num = int(len(lengh)/8)
                    else:
                        raise RuntimeError('value type is not valid')

                    place = response_i['objcontent'][0]['rowfieldtypes'][0]
                    num_elements = define_type(num, place)
                    var_data = []
                    for i in range(len(response_i['objcontent'][0]['rowvalues'])):
                        val_1 = response_i['objcontent'][0]['rowvalues'][i][2]
                        decoded = base64.b64decode(val_1)
                        a = struct.unpack(num_elements, decoded)
                        for i in a:
                            var_data.append(i)

        # we calculate the Number of dimensions and their values.

                if response_i['objkey'] == 'explorecube_dimvalues':
                    num_dimensions = num_dim(response_i, m)
                    print(num_dimensions)
                while x <= num_dimensions-1:
                    if response_i['objkey'] == 'explorecube_dimvalues':
                        dim = response_i['objcontent'][x]['title']
                        dimensions.append(response_i['objcontent'][x]['title'])
                    dims = dimensions

                    if response_i['objkey'] == 'explorecube_dimvalues' and response_i['objcontent'][x]['title'] == dim:
                        for i in response_i['objcontent'][0]['rowvalues'][0]:
                            lengh = i
                            lengh = base64.b64decode(lengh)
                            place1 = response_i['objcontent'][0]['rowfieldtypes'][0]
                        num = calculate_lengh(lengh, place1)
                        num_elements = define_type(num, place)

                        for i in response_i['objcontent'][x]['rowvalues']:

                            dim_val_1 = i[0]
                            decoded = base64.b64decode(dim_val_1)
